        ...
        <div className="flex flex-row">
            <div className="basis-11/12 flex flex-row place-items-center">
                <div className="flex flex-row cursor-default gap-[10px] font-bold text-white text-[20px]">
                    <Image src='/tournament.webp' alt='icon' width={32} height={32} />
                    <p>Турниры</p>
                </div>
            </div>
...
function Square({ img, active, timeout, name, sum, maxW = 350 }: SquareProps) {
    const link = '/tournaments/' + encodeURIComponent(name).replace(/%20/ig, '+')

    return (
        <Link href={link} target="_blank">
            <div style={{ width: maxW }} className="p-[4px] pb-0 overflow-hidden rounded-md h-[320px] bg-[#0c0c0f] relative flex flex-col border">
                <Image src={img} alt='img' width={306} height={306} className="absolute z-0 place-self-center" />
                <div className="p-[8px] z-1 opacity-95 flex flex-row justify-self-center">
                    <div>
                        <p className="bg-[#ebebeb] px-[8px] py-[2px] font-bold text-[#151515] text-[10px] text-center rounded-full">
                            {active ? 'АКТИВНЫЙ' : 'ЗАВЕРШЁННЫЙ'}
                        </p>
                    </div>
                    {active ? (
                ...
                <div className="p-[16px] z-10 bg-black opacity-95 mb-0 mt-auto">
                    ...
                    {active ? (
                        ...
                        <p className="text-center text-[#767676] text-[20px] font-bold">ЗАВЕРШЕНО</p>
                    )}
...
export function TournamentsSlider() {
    const plugData = [
        { img: '/promo.webp', active: true, timeout: '1д 14:88:66', name: 'игра галактики', sum: '322 000' },
        { img: '/plug.webp', active: false, timeout: '123', name: 'игра галактики', sum: '322 000' },
        { img: '/promo.webp', active: true, timeout: '1д 14:88:66', name: 'игра галактики', sum: '322 000' },
        { img: '/plug.webp', active: false, timeout: '123', name: 'игра галактики', sum: '322 000' },
        { img: '/promo.webp', active: true, timeout: '1д 14:88:66', name: 'игра галактики', sum: '322 000' },
        { img: '/plug.webp', active: false, timeout: '123', name: 'игра галактики', sum: '322 000' },
    ...
    return (
        <div className="flex flex-col mb-[64px] container mah-h-[520px] rounded-lg gap-[20px] pt-[12px] pb-[24px] pr-[24px] pl-[24px] bg-[#121215cc]">
            ...
            <MediaQuery maxWidth={1023}>
                <HorizontalSlider autoplay={{ delay: 60000 }} maxW={235} loop={false} spaceBetween={16} squares={plugData.map((data, i) => (
                    <Square maxW={235} key={i} img={data.img} active={data.active} timeout={data.timeout} name={data.name} sum={data.sum} />
                ))} />
            </MediaQuery>
            <MediaQuery minWidth={1024}>
                <HorizontalSlider autoplay={{ delay: 60000 }} maxW={350} loop={false} spaceBetween={16} squares={plugData.map((data, i) => (
                    <Square key={i} img={data.img} active={data.active} timeout={data.timeout} name={data.name} sum={data.sum} />
                ))} />
            </MediaQuery>
        </div>
    )
